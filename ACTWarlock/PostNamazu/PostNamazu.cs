...
namespace PostNamazu
{
    class PostNamazu
    {
        ...
        public PostNamazu(LogWriter sw)
        {
        ...
        private void Attach()
        {
            ...
            try
            {
                ...
                Log.WriteLine($"已注入FFXIV进程 {FFXIV.Id}");
            }
            catch (Exception ex)
            {
        ...
        public void DoTextCommand(string command)
        {
            if (FFXIV == null)
            {
                Log.WriteLine("执行错误：接收到指令，但是没有对应的游戏进程");
        ...
        private bool GetOffsets()
        {
            ...
            try
            {
                ...
                {
                    Log.WriteLine("无法对当前进程注入\n可能是已经被其他进程注入了？");
                    return false;
                }

                Offsets = new Offsets(scanner);
#if DEBUG
                //Log.WriteLine(Offsets.ProcessChatBoxPtr);
                //Log.WriteLine(Offsets.UiModule);
                //Log.WriteLine(Offsets.RaptureModule);
#endif
            }
            catch (ArgumentOutOfRangeException)
            {
               Log.WriteLine("查找失败：找不到特征值");
                return false;
            }
        ...
        private void ProcessChanged(Process tProcess)
        {
            if (tProcess.Id != FFXIV?.Id)
            {
                ...
                if (FFXIV != null)
                    if (GetOffsets())
                        Attach();
               Log.WriteLine($"已切换至进程{tProcess.Id}");
            }
        }
